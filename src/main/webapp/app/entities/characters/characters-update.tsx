      ...
      <Row className="justify-content-center">
        <Col md="8">
          <h2 id="lostarkApp.characters.home.createOrEditLabel" data-cy="CharactersCreateUpdateHeading">
            <Translate contentKey="lostarkApp.characters.home.createOrEditLabel">Create or edit a Characters</Translate>
          </h2>
        </Col>
      </Row>
      <Row className="justify-content-center">
        <Col md="8">
          {loading ? (
            ...
            <ValidatedForm defaultValues={defaultValues()} onSubmit={saveEntity}>
              ...
              <ValidatedField
                label={translate('lostarkApp.characters.name')}
                id="characters-name"
                name="name"
                data-cy="name"
                type="text"
              ...
              />
              <ValidatedField
                label={translate('lostarkApp.characters.advClass')}
              ...
              >
                {advClassesValues.map(advClasses => (
                  <option value={advClasses} key={advClasses}>
                    {translate('lostarkApp.AdvClasses.' + advClasses)}
                  </option>
                ))}
              </ValidatedField>
              <ValidatedField
                label={translate('lostarkApp.characters.server')}
                id="characters-server"
                name="server"
                data-cy="server"
                type="select"
              >
                {serverValues.map(server => (
                  <option value={server} key={server}>
                    {translate('lostarkApp.Server.' + server)}
                  </option>
                ))}
              </ValidatedField>
              <ValidatedField id="characters-user" name="user" data-cy="user" label={translate('lostarkApp.characters.user')} type="select">
                <option value="" key="0" />
                {users
