...
public interface CharactersRepository extends JpaRepository<Characters, Long> {
    @Query("select characters from Characters characters where characters.belongTo.login = ?#{principal.username}")
    List<Characters> findByBelongToIsCurrentUser();
    ...
    @Query(
        value = "select distinct characters from Characters characters left join fetch characters.belongTo",
    ...
    @Query("select distinct characters from Characters characters left join fetch characters.belongTo")
    List<Characters> findAllWithToOneRelationships();

    @Query("select characters from Characters characters left join fetch characters.belongTo where characters.id =:id")
