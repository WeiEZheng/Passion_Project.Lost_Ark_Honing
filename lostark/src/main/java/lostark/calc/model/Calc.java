...
public class Calc {
    ...
    public static double compareCost(Equipment equipment, Double basePercent, Double additionPercentPerFail,
        ...
        Double waitTimeDiff = simulate(basePercent, additionPercentPerFail, failLimit, 100000)
    ...
    public static Integer getExtraCost(Equipment equipment, Integer fusionMat1Amount,
                                       Integer fusionMat2Amount, Integer fusionMat3Amount) {
        if (equipment.getTier()==TierEnum.Tier1)
            return getCost(fusionMat1Amount, marketPriceService.findOneByItemName(MaterialName.StarsBreaths).get());
        else if (equipment.getTier()==TierEnum.Tier2)
            return getCost(fusionMat1Amount, marketPriceService.findOneByItemName(MaterialName.MoonsBreaths).get());
        else
            return getCost(fusionMat1Amount, marketPriceService.findOneByItemName(MaterialName.SolarGrace).get())
